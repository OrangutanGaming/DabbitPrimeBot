    ...
    async def _dev(self, ctx):
        ...
        if ctx.message.author.id == self.dev: #OGaming's User ID
            ...
            try:
                ...
                if discord.utils.get(ctx.message.author.roles, name=self.current): # User has the role
                ...
                await ctx.send(ctx.message.author.mention + " {} Needs the permission `Manage Roles`".format(self.bot.user.name))
        else:
